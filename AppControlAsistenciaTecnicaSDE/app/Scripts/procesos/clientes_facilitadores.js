...
function AgregarAsignacionAFacilitadores(cod_personal_asigna, codcliente, codsucursal, codfacilitador) {
    ...
    $('#dlgFormClientesAFacilitadores').modal();
    $('#btnGuardar').show();

    $('#btnGuardar').unbind("click");
    $('#btnGuardar').on('click', function () { return GuardarAsignacionAFacilitadores(); }); //GuardarDatosDePersonal
...
function GuardarAsignacionAFacilitadores() {
    var params = new Object();
    ...
    params = JSON.stringify(params);
    LaUrl = "/SdeAtWebService.asmx/Clientes_AFacilitadores"
    $.ajax({
        ...
        success: function (data) { //Procesar el valor del método invocado
            ...
            if (valor == "1" || valor == "true") {
                ...
                __doPostBack('ctl00$MainContent$btnActualizar_cf', "OnClick");
                $("#dlgFormClientesAFacilitadores").modal("hide");
            }
            else {
                alert(data.d.Mensaje);
            }
